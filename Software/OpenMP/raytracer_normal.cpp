...
// To compile use g++ -fopenmp raytracer_normal.cpp
...
public:
    ...
    Vec3<T> cross(const Vec3<T> &v) const { return Vec3<T>(y*v.z-v.y*z, z*v.x-v.z*x, x*v.y-v.x*y); }
    Vec3<T> operator - (const Vec3<T> &v) const { return Vec3<T>(x - v.x, y - v.y, z - v.z); }
    Vec3<T> operator + (const Vec3<T> &v) const { return Vec3<T>(x + v.x, y + v.y, z + v.z); }
...
public:
    ...
    Triangle(
        ...
        const float &transp = 0,
        const Vec3f &ec = 0) :
        vertex1(vertex1), vertex2(vertex2), vertex3(vertex3), surfaceColor(sc), emissionColor(ec),
        transparency(transp), reflection(refl)
    { /* empty */ }
    //[comment]
    // Compute a ray-triangle intersection using the geometric solution
    // Logic at https://stackoverflow.com/questions/42740765/intersection-between-line-and-triangle-in-3d
...
void render(const std::vector<Sphere> &spheres)
{
    ...
    for (unsigned y = 0; y < height; ++y) {
        for (unsigned x = 0; x < width; ++x, ++pixel) {
            ...
            Vec3f raydir(xx, yy, -1); //-1 is where eyes are at
            raydir.normalize();
...
void render2(const std::vector<Sphere> &spheres)
{
    unsigned width = 640, height = 480;
    Vec3f *image = new Vec3f[width * height], *pixel = image;
    float invWidth = 1 / float(width), invHeight = 1 / float(height);
	...
	for (unsigned y = 0; y < height; ++y) {
        for (unsigned x = 0; x < width; ++x) {
            ...
            Vec3f raydir(xx, yy, -1); //-1 is where eyes are at
            raydir.normalize();
			//#pragma omp task - worsens normal code
            pixel[y*width+x] = trace(Vec3f(0), raydir, spheres, 0);
        }
    }
	auto stop = std::chrono::high_resolution_clock::now();
	auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
	std::cout << "Hola -" << duration.count() << std::endl;


    // Save result to a PPM image (keep these flags if you compile under Windows)
    std::ofstream ofs("./raytracer2.ppm", std::ios::out | std::ios::binary);
    ofs << "P6\n" << width << " " << height << "\n255\n";
	//Parallelism is failing here..?
